...
<!--rừ vựng -->
  <div >
    <button @click="open(0)">Thêm từ vựng</button>
    <div>
      <label>Tìm kiếm từ vựng</label>
      <input v-model="keyWork" @input="searchVocabulary" placeholder="Tìm kiếm theo từ vựng, nghĩa tiếng việt, chữ viết latin"/>
    </div>
    <div>
      <label>Danh sách từ vựng</label>
      <ul>
        <li v-for="list in vocabularyList" :key="list._id">
          <div>
            <p>{{ list.pattern }}</p>
            <p>{{ list.latin }}</p>
          </div>
          ...
          <i class="bi bi-trash-fill" @click="deleteVocabulary(list._id)">Xóa</i>
        </li>
      </ul>
    </div>
  </div>
  <!--modal thêm -->
  <div class="add-vocabulary" v-if="openModal===0">
  ...
  <!-- Modal xem -->
     <div class="add-vocabulary" v-if="openModal===1">
      <label>Từ vựng</label>
      <p>{{ form.pattern }}</p>
      <label>Giải thích</label>
  ...
  <!-- Modal sửa -->
     <div class="add-vocabulary" v-if="openModal===2">
...
const addVocabulary=async()=>{
  try{
    ...
    const res=await axiosInstance.post('/vocabulary/',data)
...
const updateVocabulary=async()=>{
  try{
      ...
      if(preForm.example!== form.value.example)
        ...
        const updateVoca=await axiosInstance.patch(`/vocabulary/${preForm._id}`,data)
        if(updateVoca){
...
const getVocabulary=async()=>{
  try{
      const res=await axiosInstance.get('/vocabulary/')
      if(res){
        console.log('listVoca',res.data)
        vocabularyList.value=res.data.data
      }
  }
  catch(err){
    console.log(err)
...
const deleteVocabulary=async(id)=>{
  try{
    const res=await axiosInstance.delete(`/vocabulary/${id}`)
    if(res){
